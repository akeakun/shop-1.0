...
const CartTrigger = () => {
  ...
  return (
    <Sheet>
      ...
      <SheetContent
        asChild
        className=" bg-mainBg p-0 gap-0 w-10/12 md:max-w-[380px]"
      >
        <section className="h-full flex flex-col">
          <section className=" h-16 pb-2 mb-2 bg-mainNav text-buttonText relative">
            <h3 className="text-xl text-center font-bold uppercase px-4 py-1">
              Your Cart
            </h3>
                  ...
                  <ScrollArea className="h-full rounded-md border">
                    <div className="h-fit space-y-3">
                      {cartItems.products.map((item: any, index: any) => (
                        <div
                          key={index}
                          className="flex h-[90px] w-full relative"
                        >
                          <Link
            ...
            {cartItems.products.length === 0 && (
              <div className="my-10">
                <h4 className="text-base font-medium text-center py-4">
                  Your cart is empty🛒. Add something to cart and it will show
                  up here
                </h4>
                <Link
                  href={"/"}
